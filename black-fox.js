...
const commandsPath = path.join(__dirname, 'Module/commands');
const eventsPath = path.join(__dirname, 'Module/events');
...
function loadIslamicMessages() {
  try {
    ...
    logs.error('BLACK-FOX', 'Failed to load islamic messages:', error.message);
...
🕌 ${config.BOTNAME} | ${time} PKT`.trim();
    ...
    const cacheDir = path.join(__dirname, 'Module/commands/cache');
...
async function startBot() {
  ...
  logs.info('BOT', 'Starting BLACK FOX....');
