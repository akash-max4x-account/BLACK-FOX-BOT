  ...
  if (require.cache[resolvedPath]) {
    ...
    if (mod.children) {
      mod.children.forEach(child => {
        if (child.id.includes('Module/commands') || child.id.includes('Module/events')) {
    ...
    for (const file of files) {
      try {
        ...
        if (command.config && command.config.name) {
          ...
          logs.success('BLACK-FOX-COMMAND', `Loaded: ${command.config.name}`);
        }
      } catch (error) {
        logs.error('BLACK-FOX-COMMAND', `Failed to load ${file}: ${error.message}`);
      }
    }
    ...
    for (const file of files) {
      try {
        ...
        if (event.config && event.config.name) {
          ...
          logs.success('BLACK-FOX-EVENT', `Loaded: ${event.config.name}`);
        }
      } catch (error) {
        logs.error('BLACK-FOX-EVENT', `Failed to load ${file}: ${error.message}`);
      }
    }
